...
from DQN_Param_Share_Agent import QNParamSharingAgent
from DQN_Param_Share_Learner import QNParamSharingLeaner
...
def create_player(player,rows,columns):
    try:
        model = QNParamSharingAgent(player=player,rows=rows,columns=columns)
...
def create_learner(player,rows,columns):
    try:
        g = tf.Graph()
        with g.as_default():
            model = QNParamSharingLeaner(player=player,rows=rows,columns=columns)
