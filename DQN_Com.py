...
class QN:
    ...
    def deep_get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse) as _:
            ...
            out = tf.contrib.layers.fully_connected(out, num_outputs=num_actions*num_group_actions, activation_fn=None)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def get_best_action(self, state):
        ...
        action_values_joint = np.reshape(action_values,num_actions,num_group_actions)
        best_joint_action = np.argmax(action_values, axis=1)
        best_individual_action = best_joint_action // num_actions
        best_group_action = best_joint_action % num_actions
    ...
    def get_action(self, states, is_training=True):
        ...
        Args:
            state: observation from gym
        """
        best_individual_action, best_actions_group,_ = self.get_best_action(states)
        self.save_group_action(best_actions_group)
        ...
        # print eps
        return np.where(probs < eps, random_actions, best_individual_action)
