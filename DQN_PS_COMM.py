...
class Config():   
	...
	state_size = 3 + num_communications
...
class QN:
	...
	def deep_get_q_values_op(self, state, scope, reuse=False):
		num_actions = Config.num_actions*Config.num_communications
		out = state
		#print('state:',state)
		...
		for o in obs:
			...
			if o[0] == "loc":
				...
				if agent.left_team == o[1][0] and agent.uni_number != o[1][1]:
					state[o[1][2]-1,o[1][3]-1,self.group_comm[o[1][1]-1] + 1]=1
	...
	def train(self):
		...
		agents = []
		agent_obs = []
		self.group_comm = [0 for a in range(Config.num_players_per_team)]
		group_comm_new = [0 for a in range(Config.num_players_per_team)]
		# agents = []
		# agent_obs = []
