...
class QNParamSharingComLeaner():
    ...
    def read_sars(self):
        ...
        if(len(s)>0):
            ...
            for i in range(len(t)):
                if t[i] not in group_by_t:
                    group_by_t[t[i]]=[]
                group_by_t[t[i]].append([s[i],a[i],r[i],s_[i],Q[i]])
            
            for key in group_by_t.keys():
                #if len(group_by_t[key]) < 3: #num of agents
                #    group_by_t.pop(key, None)
                #else:
                for s_1,a_1,r_1,s__1 in self.diff_reward(group_by_t[key]):
                    s_new.append(s_1)
                    a_new.append(a_1)
                    r_new.append(r_1)
                    s__new.append(s__1)
    ...
    def diff_reward(self,group_by_t):
        global_Q=0
        diff_reward = []
        sars_updated=[]
        for i in range(len(group_by_t)):
            global_Q+=group_by_t[i][-1]
            
        for i in range(len(group_by_t)):
            diff_reward.append((global_Q - group_by_t[i][-1])/global_Q)
        
        for i in range(len(group_by_t)):
            sars_updated.append((group_by_t[i][0],group_by_t[i][1],diff_reward[i],group_by_t[i][3]))
        return sars_updated        
